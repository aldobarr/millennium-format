	...
	return (
		<div class="mx-6 mb-2 mt-6 md:mx-12 md:my-6 p-6 bg-gray-900 rounded-md">
			<Show when={!!props.page().header}>
				<header>
					<div class="prose prose-invert min-w-full flex" innerHTML={atob(props.page().header!)} />
				</header>
			</Show>
			<Show when={!!props.page().tabs && props.page().tabs!.length > 0}>
				<div class="format-body" classList={{ 'mt-8': !!props.page().header, 'mb-8': !!props.page().footer }}>
					<Show
						...
						fallback={(
							<Show when={!!props.page().tabs![0].content} fallback={<div class="prose prose-invert min-w-full flex" />}>
								<div class="prose prose-invert min-w-full flex" innerHTML={atob(props.page().tabs![0].content)} />
							</Show>
						)}
					>
						<Tabs class="tabs">
							...
							<For each={props.page().tabs}>
								{(tab, index) => (
									<Tabs.Content class="tabs__content" value={`tab-${index()}`}>
										<div class="prose prose-invert min-w-full flex" innerHTML={atob(tab.content)} />
									</Tabs.Content>
								)}
							</For>
						</Tabs>
